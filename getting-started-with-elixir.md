...
* `add(x, l)`: add the element _x_ to the list _l_ if it is not in the list
* `remove(x, l)`: remove all occurrences of _x_ in _l_
* `unique(l)`: return a list of unique elements in the list _l_, that is are the unique elements in the list
* `pack(l)`: return a list containing lists of equal elements, `[:a, :a, :b, :c, :b, :a, :c]` should return , `[[:a, :a, :a], [:b, :b], [:c, :c]]`
